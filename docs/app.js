...
/**
 * Build adjacency list: predId → [successorId, ...]
 * A task lists its prerequisites in Predecessor_IDs.
 */
...
/** Kahn's topological sort. Returns node order or partial order if cycle exists. */
...
/**
 * Adjusted WSJF = Base + 0.5 × sum of all downstream Base WSJFs.
 * This is the domino effect: a tiny enabling task inherits the value
 * of everything it unblocks.
 */
...
/** CPM forward pass. Returns { path: [id,...], dur: days } */
function critPath(graph, idx) {
  const order = topoSort(graph);
  if (!order.length) return { path: [], dur: 0 };

  // Build reverse graph for traceback
  const rev = {};
  ...
  // Trace back from latest-finishing node
 ...
 * 1. Compute Base_WSJF for every task
 * 2. Compute Adjusted_WSJF (domino rollup)
 * 3. Flag bottlenecks, decomposition needs, critical path
 * 4. Sort active tasks by Adjusted_WSJF descending
 */
...
function analyze() {
  ...
  // Run in a microtask so the spinner renders first
  setTimeout(() => {
    try {
...
function seedData() {
  ...
  // Pre-assign IDs and compute Base_WSJF
  DB.save([]);
  demos.forEach((d, i) => {
...
function init() {
  ...
  // Add domino-tag style (not in CSS to keep it inline)
